/**
 * Binds the legacy module surface to a window global, and emits a shim for each
 * legacy path so an extension's `import { x } from "/scripts/app.js"` keeps
 * resolving after the module itself moved into the bundle.
 *
 * `window.comfyAPI` is NOT a leftover. Nothing in src/ reads it — it exists only
 * for third-party extensions, and every one of them written against the upstream
 * project reaches for that exact name. Renaming it would rebrand a key at the
 * cost of every extension that already exists, so the brand lives in the console
 * notices below, where a person reads it, and the runtime key stays a contract.
 */
...
function getWarningMessage(
  ...
  if (isDeprecated) {
    return `[Hanzo Studio Deprecated] Importing from "${shimFileName}" is deprecated and will be removed in v1.34.`
  }

  return `[Hanzo Studio Notice] "${shimFileName}" is an internal module, not part of the public API. Future updates may break this import.`
}
